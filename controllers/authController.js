...
exports.verifySeller = catchAsync(async (req, res, next) => {
  const { idDocument, vatDocument, vatNumber } = req.body;
...
exports.confirmVerification = catchAsync(async (req, res, next) => {
  ...
  if (!verification) {
    return next(
      res.status(404).json({
        status: "Not found",
        message: `No pending verification with id: ${id}`,
      })
    );
  }
  ...
  if (status === "Confirmed") {
    ...
    try {
      ...
      await new Email(updatedUser, resetURL).verificationMail();
    } catch (error) {
      return next(
        res.status(500).json({
          status: "Server error!",
